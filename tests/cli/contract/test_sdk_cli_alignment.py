...
def assert_cli_sdk_alignment(
    ...
    *,
    ...
    """
    exclude_cli = (exclude_cli or set()) | CLI_GLOBALS | PAGINATION_OPTIONS
...
SDK_EXCLUSIONS = {
    ...
    "buckets_download": {"key"},  # CLI uses name, not key
    "buckets_upload": {"key", "content", "content_type"},  # CLI uses name + file path
}
...
@pytest.mark.parametrize(
    ...
    [
        # Phase 1 - Buckets (tracer bullet)
        ("buckets", "list", "BucketsService", "list"),
        ("buckets", "retrieve", "BucketsService", "retrieve"),
        ("buckets", "create", "BucketsService", "create"),
        # Note: delete command excluded - uses bucket.delete() resource pattern, not service method
        ("buckets", "download", "BucketsService", "download"),
        ("buckets", "upload", "BucketsService", "upload"),
