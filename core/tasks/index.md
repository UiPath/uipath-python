...
| `task_source_metadata`          | \`dict[str, Any] | None\`                                                             | Optional extra keys merged into taskSource.taskSourceMetadata, on top of the built-in InstanceId/FolderKey/JobKey/ProcessKey (e.g. a conversational-agent id a HITL task should carry downstream). Takes effect only when taskSource itself is populated, which needs both UiPathConfig.project_id and UiPathConfig.trace_id set; otherwise this (like the rest of taskSource) is silently dropped. |
