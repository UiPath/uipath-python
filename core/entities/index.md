...
```
create_entity_v3(name, fields, options=None)
```

Create an entity via the v3 API (supports Federated entities).

Experimental v3 surface (serves Federated entities); see :meth:`create_entity` for parameter and return details.

### create_entity_v3_async

```
create_entity_v3_async(name, fields, options=None)
```
...
```
delete_entity_v3(entity_id)
```

Delete an entity via the v3 API.

Experimental v3 surface (serves Federated entities); see :meth:`delete_entity` for parameter and return details.

### delete_entity_v3_async

```
delete_entity_v3_async(entity_id)
```
...
```
delete_record_v3(entity_key, record_id)
```

Delete a single record by id via the v3 API.

Experimental v3 surface (serves Federated entities); see :meth:`delete_record` for parameter and return details.

### delete_record_v3_async

```
delete_record_v3_async(entity_key, record_id)
```
...
Experimental v3 surface (serves Federated entities); see :meth:`delete_records` for parameter and return details.

### delete_records_v3_async
...
```
get_ontology_file_async(
    ontology_name, file_type="owl", folder_key=None
)
```
...
This method is currently experimental. Behavior and parameters are subject to change in future versions.

Parameters:

| Name            | Type  | Description                                                              | Default                                  |
| --------------- | ----- | ------------------------------------------------------------------------ | ---------------------------------------- |
| `ontology_name` | `str` | Name of the ontology.                                                    | *required*                               |
| `file_type`     | `str` | The ontology file to fetch — one of owl, r2rml, shacl, summary, context. | `'owl'`                                  |
| `folder_key`    | \`str | None\`                                                                   | Key of the folder the ontology lives in. |

Returns:

| Type             | Description                                                  |
| ---------------- | ------------------------------------------------------------ |
| `dict[str, Any]` | Dict\[str, Any\]: The file record (e.g. content, mediaType). |
...
```
get_record_v3(entity_key, record_id, expansion_level=None)
```

Fetch a single record by id via the v3 API.

Experimental v3 surface (serves Federated entities); see :meth:`get_record` for parameter and return details.
...
```
insert_record_v3(entity_key, data, expansion_level=None)
```

Insert a single record via the v3 API.

Experimental v3 surface (serves Federated entities); see :meth:`insert_record` for parameter and return details.

### insert_record_v3_async

```
insert_record_v3_async(
    entity_key, data, expansion_level=None
)
```
...
Experimental v3 surface (serves Federated entities); see :meth:`insert_records` for parameter and return details.

### insert_records_v3_async
...
Experimental v3 surface (serves Federated entities); see :meth:`list_entities` for parameter and return details.

### list_entities_v3_async
...
To filter, sort, or project, use :meth:`retrieve_records` — this endpoint only pages::
...
To filter, sort, or project, use :meth:`retrieve_records_async` — this endpoint only pages.
...
Experimental v3 surface (serves Federated entities); see :meth:`list_records` for parameter and return details.

### list_records_v3_async
...
query_entity_records(
    sql_query,
    *,
    relationships_as_scalar=False,
    resolve_choice_sets=False,
)
```
...
| `relationships_as_scalar` | `bool` | When True, relationship (FK) fields are typed as their underlying scalar id instead of VARIANT, so a query can join on relationshipField = Other.Id. Sent as queryOptions.relationshipsAsScalar in the request body. Defaults to False (unchanged behaviour). | `False`    |
| `resolve_choice_sets`     | `bool` | When True, choice-set fields in results are returned as key-value pairs (label + NumberId) instead of bare integers. Sent as queryOptions.resolveChoiceSets in the request body. Defaults to False.                                                           | `False`    |
...
### resolve_entity_set_v3

```
resolve_entity_set_v3(items)
```

Resolve an agent entity set via the v3 API (serves Federated entities).

Experimental v3 surface; behaves like :meth:`resolve_entity_set` but fetches entity metadata from `datafabric_/api/v3/entities` so Federated entities resolve with their external field definitions.

### resolve_entity_set_v3_async

```
resolve_entity_set_v3_async(items)
```
...
```
retrieve_by_name_v3(entity_name, folder_key=None)
```

Retrieve an entity by name via the v3 API.

Experimental v3 surface (serves Federated entities); see :meth:`retrieve_by_name` for parameter and return details.

### retrieve_by_name_v3_async

```
retrieve_by_name_v3_async(entity_name, folder_key=None)
```
...
Experimental v3 surface (serves Federated entities); see :meth:`retrieve_records` for parameter and return details.

### retrieve_records_v3_async
...
```
retrieve_v3(entity_key)
```

Retrieve an entity by key via the v3 API.

Experimental v3 surface (serves Federated entities); see :meth:`retrieve` for parameter and return details.

### retrieve_v3_async

```
retrieve_v3_async(entity_key)
```
...
### update_entity_metadata_v3

```
update_entity_metadata_v3(entity_id, metadata)
```

Update entity metadata via the v3 API.

Experimental v3 surface (serves Federated entities); see :meth:`update_entity_metadata` for parameter and return details.

### update_entity_metadata_v3_async

```
update_entity_metadata_v3_async(entity_id, metadata)
```
...
Experimental v3 surface (serves Federated entities); see :meth:`update_record` for parameter and return details.

### update_record_v3_async
...
Experimental v3 surface (serves Federated entities); see :meth:`update_records` for parameter and return details.

### update_records_v3_async
