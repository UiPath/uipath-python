...
The server creates the ephemeral index and starts the Deep RAG task in one operation; the caller does not need to create the index separately.
...
| `name`          | `str`          | The name of the Deep RAG task.                                                                    | *required* |
| `prompt`        | `str`          | Describe the task: what to research across documents, what to synthesize and how to cite sources. | *required* |
| `attachments`   | `list[str]`    | The list of attachment ids from which the ephemeral index will be created.                        | *required* |
| `citation_mode` | `CitationMode` | The citation mode to use. Defaults to INLINE.                                                     | `INLINE`   |
| `folder_key`    | `str`          | The folder key context. Defaults to None.                                                         | `None`     |
| `folder_path`   | `str`          | The folder path context. Defaults to None.                                                        | `None`     |
...
### start_deep_rag_from_attachments_async
...
| `search_during_ingestion` | `bool`               | If True, query the index even while an ingestion is in progress, returning results from the documents indexed so far. Defaults to False, which raises IngestionInProgressException instead. | `False`                                         |
...
| Type                           | Description                                                           |
| ------------------------------ | --------------------------------------------------------------------- |
| `IngestionInProgressException` | If the index is still ingesting and search_during_ingestion is False. |
