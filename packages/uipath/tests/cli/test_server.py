...
class TestServer:
    ...
    def test_start_job_that_faults_is_not_reported_successful(
        self, server, temp_dir, failing_script
    ):
        """A job that runs and faults must say so in the body.

        The response is still 200 — the request succeeded, the job did not — so the
        body is the only signal. Click returns ctx.exit(1) rather than raising under
        standalone_mode=False, so an exit code that is not threaded through lands here
        as a successful job.
        """
        port = server
        job_key = "test-job-faulted"
