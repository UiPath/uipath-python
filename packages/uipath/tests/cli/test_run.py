...
class TestRun:
    ...
    class TestMiddleware:
        ...
        def test_installs_workspace_runtimes_after_simulation_before_chat_runtime(
            self,
            runner: CliRunner,
            temp_dir: str,
            ...
            base_runtime.get_schema = AsyncMock(return_value=Mock(metadata=None))
            storage = Mock()
            ...
            mock_runtime = Mock(dispose=AsyncMock())
            ...
            with runner.isolated_filesystem(temp_dir=temp_dir):
                ...
                with (
                    ...
                    patch(
                        "uipath._cli._managed_workspace.UiPath",
                        return_value=client,
                    ),
                    patch(
                    ...
                    patch(
                        "uipath._cli.cli_run.UiPathMockRuntime",
                        return_value=mock_runtime,
                    ) as mock_runtime_type,
            ...
            assert wrapped_runtime.delegate.delegate is mock_runtime
            assert mock_runtime_type.call_args.kwargs["delegate"] is base_runtime
            assert (
                wrapped_runtime.registry_store
                is wrapped_runtime.delegate.registry_store
            )
            assert (
        ...
        def test_suspended_workspace_takes_precedence_over_conversation_snapshot(
            ...
            job_key = UUID(int=3)
            deleted_attachment_key = UUID(int=5)
            attachment_contents = {
                conversation_attachment_key: b"conversation",
                suspended_attachment_key: b"suspended",
                deleted_attachment_key: b"deleted before suspension",
            }
            ...
            async def stream_runtime(*_, **__):
                observed_contents.append(
                    (
                        (get_workspace_path() / "notes.txt").read_text(
                            encoding="utf-8"
                        ),
                        (get_workspace_path() / "deleted.txt").exists(),
                    )
                )
                ...
                "uipath__conversation_meta_events": [
                    {
                        "metaEvent": {
                            "workspaceFiles": [
                                ...
                                {
                                    "path": "deleted.txt",
                                    "attachmentKey": str(deleted_attachment_key),
                                },
                            ]
                        }
                    }
            ...
            assert observed_contents == [("suspended", False)]
            downloaded_keys = [
                call.kwargs["key"]
                for call in attachments.download_async.await_args_list
            ]
            assert downloaded_keys == [suspended_attachment_key]
            base_runtime.dispose.assert_awaited_once()
