...
class AuthSession:
    ...
    def get_tenants_and_organizations(self) -> TenantsAndOrganizationInfoResponse:
        ...
        try:
            self._tenants_and_organizations = (
                PlatformPortalService.get_tenants_and_organizations(
                    domain=self.domain,
                    prt_id=self.prt_id,
    ...
    def refresh_access_token(self, refresh_token: str) -> TokenData:
        client_id = OidcUtils.get_auth_config(self.domain)["client_id"]
        try:
            return IdentityService.refresh_access_token(
                domain=self.domain,
    ...
    def enable_studio_web(self, base_url: str) -> None:
        if not self.access_token:
            raise ValueError("Cannot enable StudioWeb: access_token must be set.")
    ...
    def _retrieve_tenant(self, tenant_name: str):
        data = self.get_tenants_and_organizations()
        organization = data.organization
        tenants = data.tenants
