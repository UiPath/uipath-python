...
class _ServerState:
    ...
    def __init__(self) -> None:
        ...
        # Optional per-job scope, registered by an out-of-process concern that needs to
        # bracket every job this server runs. ``None`` — the default — means no scope is
        # installed and the job runs exactly as it did before. Set here rather than in
        # ``init`` so a provider registered before server startup is not cleared.
