"""CLI command for visualizing agent trajectory traces.
...
class SpanNode:
    ...
    def duration_ms(self) -> float | None:
        ...
        if start is None or end is None:
            return None
        try:
            t0 = _parse_otel_time(start)
            t1 = _parse_otel_time(end)
...
def _parse_otel_time(value: Any) -> datetime:
...
def _build_tree_from_jsonl(spans: list[dict[str, Any]]) -> list[SpanNode]:
...
def _build_tree_from_eval(spans: list[dict[str, Any]]) -> list[SpanNode]:
...
def _build_tree(spans: list[dict[str, Any]], is_eval: bool) -> list[SpanNode]:
    """Build a span tree from loaded spans."""
    if is_eval:
        return _build_tree_from_eval(spans)
...
def _render_span_node(
    ...
    *,
    ...
    parts.append(status)
    if span_type and show_full:
        parts.append(f"[dim]\\[{span_type}][/dim]")
    ...
    for event in node.events:
        ...
        if event_name == "exception":
            ...
            branch.add(
                f"[red]⚠ {exc_type}: {_truncate(str(exc_msg), max_value_length)}[/red]"
            )
...
def _render_key_attributes(
    ...
    *,
    ...
    if show_input:
        ...
        if input_val:
            parsed = _safe_parse_json(input_val)
            display = _truncate(
    ...
    if show_output:
        ...
        if output_val:
            parsed = _safe_parse_json(output_val)
            display = _truncate(
                json.dumps(parsed, default=str)
                if isinstance(parsed, (dict, list))
                else str(parsed),
...
def _render_all_attributes(
    ...
    for key, value in sorted(attrs.items()):
        display_value = _safe_parse_json(value) if isinstance(value, str) else value
        if isinstance(display_value, (dict, list)):
            display_str = _truncate(
                json.dumps(display_value, default=str), max_value_length
            )
        else:
            display_str = _truncate(str(display_value), max_value_length)
    ...
    *,
    ...
    if trace_groups:
        for trace_id, group_roots in trace_groups.items():
            ...
            tree = Tree(f"[bold magenta]Trace[/bold magenta] [dim]{display_id}[/dim]")
        ...
        tree = Tree("[bold magenta]Trace[/bold magenta]")
        for root in ungrouped:
            _render_span_node(
                tree,
                root,
                show_input=show_input,
    ...
    # Summary
    total_spans = _count_spans(roots)
...
def _count_spans(roots: list[SpanNode]) -> int:
    ...
    for root in roots:
        count += 1
        count += _count_spans(root.children)
    return count
...
def trace(
    ...
    Examples:
        ...
        uipath trace traces.jsonl --name "get_weather"
        uipath trace traces.jsonl --span-type TOOL
        uipath trace traces.jsonl --status error
        uipath trace traces.jsonl --contains "get_random*"
        uipath trace eval_output.json --eval-id "test-case-1"
        uipath trace traces.jsonl --full
    """  # noqa: D301
    ...
    roots = _build_tree(spans, is_eval=is_eval)

    # --contains: keep full subtrees that contain a matching span
    if contains_pattern:
        roots = _filter_contains(roots, contains_pattern)
