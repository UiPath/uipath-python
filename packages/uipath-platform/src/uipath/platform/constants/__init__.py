...
Single source of truth for constants shared across the UiPath SDK packages.
Lives directly under ``uipath.platform`` (sibling to ``common``) so it can be
imported cheaply — ``uipath.platform``'s ``__init__`` resolves its heavy
exports lazily, so importing this module does not pull in the service layer.

``uipath.platform.common.constants`` and ``uipath._utils.constants`` re-export
from here.
"""
