...
1. **LLM Judge Strict JSON Similarity Output Evaluator** (`llm-judge-output-strict-json-similarity`): Strict JSON structure comparison with LLM judgment
...
1. **Per-Key Matching**: For each expected key, checks if it exists in actual output
1. **Content Assessment**: For matching keys, evaluates content similarity (identical/similar/different)
1. **Penalty-Based Scoring**: Calculates score using these penalties per key:
...
1. **Craft clear prompts** - Be specific about evaluation criteria
1. **Include both placeholders** - Always use `{{ActualOutput}}` and `{{ExpectedOutput}}`
1. **Set score thresholds** - Define minimum acceptable scores (e.g., ≥ 0.8)
1. **Review justifications** - Use LLM explanations to understand scores
1. **Cost awareness** - LLM evaluations use API calls, consider token costs
...
- [LLM Judge Trajectory Evaluator](https://uipath.github.io/uipath-python/eval/llm_judge_trajectory/index.md): For evaluating agent execution paths
- [JSON Similarity Evaluator](https://uipath.github.io/uipath-python/eval/json_similarity/index.md): For deterministic JSON comparison
- [Exact Match Evaluator](https://uipath.github.io/uipath-python/eval/exact_match/index.md): For strict string matching
- [Contains Evaluator](https://uipath.github.io/uipath-python/eval/contains/index.md): For substring matching
