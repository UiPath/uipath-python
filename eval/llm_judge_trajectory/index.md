...
1. **LLM Judge Trajectory Simulation Evaluator** (`llm-judge-trajectory-simulation`): Specialized for tool simulation scenarios
...
1. **Agent Unawareness**: The agent doesn't know tool responses are simulated
1. **Controlled Testing**: Allows testing agent behavior with predictable tool responses
1. **Evaluation Focus**: Assesses whether the agent behaves correctly given the simulated tool responses
...
1. **Use temperature 0.0** for consistent evaluations
1. **Include context** - Provide enough detail in expected behavior
1. **Consider partial credit** - LLM can give partial scores for mostly correct trajectories
1. **Review justifications** - Understand why trajectories scored high or low
1. **Combine with tool evaluators** - Use [Tool Call Evaluators](https://uipath.github.io/uipath-python/eval/tool_call_order/index.md) for strict ordering requirements
...
1. **Decision points**: When the agent should make choices
1. **Conditional logic**: "If X, then Y" scenarios
1. **Success criteria**: What constitutes good behavior
1. **Error handling**: How agent should handle failures
...
- [LLM Judge Output Evaluator](https://uipath.github.io/uipath-python/eval/llm_judge_output/index.md): For evaluating outputs instead of processes
- [Tool Call Order Evaluator](https://uipath.github.io/uipath-python/eval/tool_call_order/index.md): For strict deterministic sequence validation
- [Tool Call Count Evaluator](https://uipath.github.io/uipath-python/eval/tool_call_count/index.md): For validating tool usage frequencies
- [Tool Call Args Evaluator](https://uipath.github.io/uipath-python/eval/tool_call_args/index.md): For validating tool arguments
