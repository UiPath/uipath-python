...
1. Use strict or JSON similarity comparison based on mode
...
1. **Use strict mode for critical outputs** - Boolean flags, status codes, exact values
1. **Validate key fields** - Focus on important output fields
1. **Test error scenarios** - Verify tools return proper error outputs
1. **Consider data types** - Ensure strings, numbers, booleans are correct
1. **Test edge cases** - Empty responses, null values, error conditions
1. **Combine with other evaluators** - Use with [Tool Call Args](https://uipath.github.io/uipath-python/eval/tool_call_args/index.md) for complete validation
...
1. **Order matters** - First expected output matches first actual call of that tool
1. **No temporal validation** - Doesn't verify when outputs occurred
...
- [Tool Call Args Evaluator](https://uipath.github.io/uipath-python/eval/tool_call_args/index.md): Validates tool arguments (inputs)
...
- [Output Evaluator](https://uipath.github.io/uipath-python/eval/#output-based-evaluators): For final agent outputs
