...
1. **Leaf Comparison**: Only leaf nodes (actual values) are compared, using type-specific similarity measures:
...
1. **Set score thresholds** based on your tolerance for differences (e.g., require score ≥ 0.9)
1. **Combine with exact match** for critical fields that must match exactly
1. **Only expected keys matter** - extra keys in actual output are ignored
1. **Consider array order** - elements are compared by position
1. **Useful for API testing** where responses may have minor variations
...
- [Exact Match Evaluator](https://uipath.github.io/uipath-python/eval/exact_match/index.md): For strict matching
- [Contains Evaluator](https://uipath.github.io/uipath-python/eval/contains/index.md): For substring matching
- [LLM Judge Output Evaluator](https://uipath.github.io/uipath-python/eval/llm_judge_output/index.md): For semantic comparison
