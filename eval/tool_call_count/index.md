...
1. **Combine with order validation** - Use with [Tool Call Order Evaluator](https://uipath.github.io/uipath-python/eval/tool_call_order/index.md) for complete validation
1. **Set realistic bounds** - Use `<=` and `>=` for flexible but bounded behavior
1. **Use strict mode sparingly** - Non-strict provides better debugging information
1. **Consider variability** - Use ranges (`>=`, `<=`) when exact counts might vary
1. **Test efficiency** - Ensure agents don't make redundant calls
...
1. **No temporal information** - Doesn't know when calls happened
1. **No context awareness** - Doesn't understand why counts differ
1. **All tools independent** - Each tool evaluated separately
...
- [LLM Judge Trajectory Evaluator](https://uipath.github.io/uipath-python/eval/llm_judge_trajectory/index.md): For semantic evaluation
