...
1. Compare arguments based on mode (strict or JSON similarity)
1. Calculate score based on match quality
...
1. **Use subset mode for flexibility** - When some arguments are optional or variable
1. **Combine with order validation** - Use with [Tool Call Order Evaluator](https://uipath.github.io/uipath-python/eval/tool_call_order/index.md)
1. **Test critical parameters in strict mode** - For security, authentication, or business-critical params
1. **Validate data types** - Ensure strings, numbers, booleans are used correctly
1. **Test edge cases** - Empty strings, null values, zero, negative numbers
1. **Consider partial matches** - Non-strict mode gives partial credit for similar values
...
1. **Order matters for multiple calls** - First expected call matches first actual call of that name
1. **No cross-tool validation** - Each tool evaluated independently
1. **Case-sensitive** - Keys and values are case-sensitive
...
- [Tool Call Order Evaluator](https://uipath.github.io/uipath-python/eval/tool_call_order/index.md): Validates tool call sequences
...
- [JSON Similarity Evaluator](https://uipath.github.io/uipath-python/eval/json_similarity/index.md): For general JSON comparison
