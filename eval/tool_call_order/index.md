...
1. **Use non-strict mode for flexible workflows** - When some steps might be optional or reorderable
1. **Combine with other evaluators** - Use with [Tool Call Count](https://uipath.github.io/uipath-python/eval/tool_call_count/index.md) and [Tool Call Args](https://uipath.github.io/uipath-python/eval/tool_call_args/index.md)
1. **Be specific with tool names** - Ensure tool names in criteria match exactly (case-sensitive)
1. **Consider repeated calls** - If a tool should be called multiple times, list it multiple times
1. **Test partial workflows** - Use non-strict mode during development, strict in production
...
1. **No argument validation** - Only checks tool names, not arguments (use [Tool Call Args Evaluator](https://uipath.github.io/uipath-python/eval/tool_call_args/index.md))
1. **Position-based** - Doesn't consider parallel execution
1. **Multiple calls** - Each call is treated as separate position in sequence
...
- [Tool Call Count Evaluator](https://uipath.github.io/uipath-python/eval/tool_call_count/index.md): Validates tool usage frequencies
- [Tool Call Args Evaluator](https://uipath.github.io/uipath-python/eval/tool_call_args/index.md): Validates tool arguments
- [Tool Call Output Evaluator](https://uipath.github.io/uipath-python/eval/tool_call_output/index.md): Validates tool outputs
- [LLM Judge Trajectory Evaluator](https://uipath.github.io/uipath-python/eval/llm_judge_trajectory/index.md): For semantic trajectory evaluation
