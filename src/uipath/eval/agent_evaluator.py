...
class AgentEvaluator:
    ...
    async def run(
        ...
        Args:
            expected_output: The expected output from the agent
        ...
        async for eval_result in self._run_evaluators(
            agent_execution_output, agent_input, expected_output
        ):
            yield eval_result
        ...
        for evaluator in self._evaluators:
            try:
                result = await evaluator.evaluate(
                    ...
                    expected_output=self._extract_target_output(
                        expected_output, evaluator.target_output_key
                    ),
    ...
    async def run_and_collect(
        ...
        async for result in self.run(expected_output, agent_input):
