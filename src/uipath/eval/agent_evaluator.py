    ...
    async def _run_evaluators(
        ...
        for evaluator in self._evaluators:
            try:
                ...
                # fallback to evaluator class name
                criteria = evaluation_criteria.get(type(evaluator))
                if not criteria:
                    # Try to find by class name in case it's a coded evaluator with same name
                    evaluator_class_name = type(evaluator).__name__
                    for key in evaluation_criteria.keys():
                        if hasattr(key, '__name__') and key.__name__ == evaluator_class_name:
                            criteria = evaluation_criteria[key]
                            break
