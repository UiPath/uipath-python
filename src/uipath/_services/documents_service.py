...
class DocumentsService(FolderContext, BaseService):
    ...
    def extract(
        ...
        with open(Path(file_path).resolve(), "rb") if file_path else nullcontext() as f:
            file = f if file_path else file
            document_id = self._start_digitization(project_id=project_id, file=file)
    ...
    def create_validation_action(
        ...
        """Create a validation action for a document based on the extraction response. More details about validation actions can be found in the [official documentation](https://docs.uipath.com/ixp/automation-cloud/latest/overview/working-with-validation-actions).
