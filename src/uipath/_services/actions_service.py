...
from .._utils.constants import ENV_TENANT_ID, HEADER_TENANT_ID, HEADER_FOLDER_KEY, HEADER_FOLDER_PATH
...
def _retrieve_action_spec(action_key: str, app_folder_key: str, app_folder_path: str) -> RequestSpec:
    return RequestSpec(
        method="GET",
...
class ActionsService(FolderContext, BaseService):
    ...
    def create(
        ...
        *,
        ...
        response = self.request(spec.method, spec.endpoint, content=spec.content, headers=spec.headers)
        json_response = response.json()
        if assignee:
            spec = _assign_task_spec(json_response["id"], assignee)
    ...
    async def __get_app_key_and_schema_async(
        ...
        try:
            ...
            raise Exception("Action app not found")
        try:
            return (
                deployed_app_key,
                ActionSchema(
            ...
            raise Exception("Failed to deserialize action schema")
