...
async def execute_parallel(
    evaluation_result_iterable: Iterable[Awaitable[EvaluationRunResult]],
    workers: int,
) -> list[EvaluationRunResult]:
    semaphore: asyncio.Semaphore = asyncio.Semaphore(workers)

    async def execute_with_semaphore(
        eval_item: Awaitable[EvaluationRunResult],
    ) -> EvaluationRunResult:
        async with semaphore:
            return await eval_item

    return await asyncio.gather(
        *(execute_with_semaphore(item) for item in evaluation_result_iterable)
    )
