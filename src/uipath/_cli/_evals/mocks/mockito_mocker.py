...
class MockitoMocker(Mocker):
    ...
    def __init__(self, mocking_strategy: MockingStrategy):
        ...
        assert isinstance(self.mocking_strategy, MockitoMockingStrategy)

        self.stub = Stub()
        mock_obj = mocking.Mock(self.stub)

        for behavior in self.mocking_strategy.behaviors:
    ...
    async def response(
        ...
        if not isinstance(self.mocking_strategy, MockitoMockingStrategy):
