...
from uipath._cli._evals.mocks.strategy import MockingStrategy
...
def set_execution_context(
    mocking_strategy: MockingStrategy | None,
    span_collector: ExecutionSpanCollector,
    execution_id: str,
) -> None:
    """Set the execution context for an evaluation run for mocking and trace access."""
    mocking_strategy_context.set(mocking_strategy)

    try:
        if mocking_strategy:
            mocker_context.set(MockerFactory.create(mocking_strategy))
        else:
...
def clear_execution_context() -> None:
    ...
    mocking_strategy_context.set(None)
    mocker_context.set(None)
