...
def update_agent_json(
    ...
    # Try to download existing agent.json
    ...
    if "agent.json" in remote_files:
        file_url = f"{base_api_url}/File/{remote_files['agent.json'].id}"
        response = (
    ...
    if "agent.json" in remote_files:
        url = f"{base_api_url}/File/{remote_files['agent.json'].id}"
        response = (
...
def upload_source_files_to_project(
    ...
    with httpx.Client(**get_httpx_client_kwargs()) as client:
        ...
        try:
            update_agent_json(
                project_id, base_url, token, directory, client, processed_files
            )
        except Exception as e:
