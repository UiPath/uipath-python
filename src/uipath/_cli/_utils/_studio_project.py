...
from uipath._utils.constants import HEADER_SW_LOCK_KEY, ENV_TENANT_ID, HEADER_TENANT_ID
...
class ResourceOverwrite(BaseModel):
    ...
    Attributes:
        ...
        folder_path: Optional folder path for the resource
    ...
    kind: str
    overwrite_name: str
    overwrite_folder_path: str


class BindingOverwrites(BaseModel):
    """Represents the API response for binding overwrites.

    The response is a dictionary where keys are in the format 'kind.resource_name'
    and values contain the overwrite details. This model parses the response and
    creates a list of ResourceOverwrite objects.
    """
    model_config = ConfigDict(
        populate_by_name=True,
    )

    overwrites: List[ResourceOverwrite] = Field(default_factory=list)

    @classmethod
    def from_dict(cls, data: dict[str, dict]) -> "BindingOverwrites":

        overwrites = []
        for key, value in data.items():
            # Parse the key format: "kind.resource_name"
            parts = key.split(".", 1)
            if len(parts) != 2:
                continue

            kind, resource_name = parts

            overwrite_data = OverwriteData.model_validate(value)

            overwrite = ResourceOverwrite(
                name=resource_name,
                kind=kind,
                overwrite_name=overwrite_data.name,
                overwrite_folder_path=overwrite_data.folder_path
            )
            overwrites.append(overwrite)

        return cls(overwrites=overwrites)
...
class StudioClient:
    ...
    async def get_overwrites(self, bindings_file_path: Path) -> List[ResourceOverwrite]:
        """Get resource overwrites from the solution.

        Args:
            bindings_file_path: Path to the bindings file

        Returns:
            List[ResourceOverwrite]: List of resource overwrites with their configurations
        """
        solution_id = await self._get_solution_id()
        tenant_id = os.getenv(ENV_TENANT_ID, None)

        # Read the bindings file
        with open(bindings_file_path, "rb") as f:
            file_content = f.read()

        # Create multipart files data
        files = [
            ("file", (os.path.basename(bindings_file_path), file_content, "application/json"))
        ]
        ...
        # Parse and validate the response using the BindingOverwrites model
        binding_overwrites = BindingOverwrites.from_dict(response.json())

        return binding_overwrites.overwrites

    async def create_virtual_resource(self, virtual_resource_request: VirtualResourceRequest) -> VirtualResourceResult:
        ...
        if virtual_resource_request.type:
            base_message_parts.append(f", type: {click.style(virtual_resource_request.type, fg='yellow')}")

        if virtual_resource_request.activity_name:
            base_message_parts.append(f", activity: {click.style(virtual_resource_request.activity_name, fg='yellow')}")
        ...
        existing_same_kind = next(
            (r for r in existing_resources if r["name"] == virtual_resource_request.name and r["kind"] == virtual_resource_request.kind),
            None
        )
        if existing_same_kind:
            message = f"{base_message} already exists. Skipping..."
            return VirtualResourceResult(
        ...
        await self._update_resource_specs(resource_key,
                                          new_specs={"name": virtual_resource_request.name})

        # Update cache with newly created resource
        self._resources_cache.append({"name": virtual_resource_request.name, "kind": virtual_resource_request.kind})
    ...
    async def _update_resource_specs(self, resource_key: str, new_specs: dict[str,Any]):
        solution_id = await self._get_solution_id()
        tenant_id = os.getenv(ENV_TENANT_ID, None)
