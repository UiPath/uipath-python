...
def format_output(
    ...
    Example:
        ...
        bucket2
    """
    # Handle iterators and generators FIRST (before Pydantic check)
    ...
    # Convert Pydantic models to dicts (AFTER iterator conversion)
    if hasattr(data, "model_dump"):
        data = data.model_dump()
    elif isinstance(data, list) and len(data) > 0 and hasattr(data[0], "model_dump"):
        data = [item.model_dump() for item in data]

    # Explicitly handle async iterators (not supported)
    ...
    # Force colorless output when writing table to file
    ...
    # Output to file or stdout (SEPARATES LOGS FROM DATA)
    if output:
        Path(output).write_text(text)
        click.echo(f"Output written to {output}", err=True)  # Log to stderr
    else:
        click.echo(text)  # Data to stdout
...
def _format_table(data: Any, no_color: bool = False) -> str:
    ...
    # Try to use rich, fallback to simple table if not available
    try:
