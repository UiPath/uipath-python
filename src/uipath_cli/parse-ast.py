        ...
        for target in node.targets:
            if isinstance(target, ast.Name):
                if not isinstance(node.value, ast.Constant):
                    self.env[target.id] = [ast.get_source_segment(source, node.value), t]
                else:
...
class AssetManagerCallVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node):
        ...
        attrId = node.func.value.attr if isinstance(node.func.value, ast.Attribute) else node.func.value.id
        func = node.func.value if isinstance(node.func.value, ast.Name) else node.func.value.value
        if isinstance(func, ast.Name) and attrId == "AssetManager":
            if method == "setAsset" and len(node.args) >= 2:
                name_node = node.args[0]
                value_node = node.args[1]
                if (not isinstance(name_node, ast.Constant) and not isinstance(name_node, ast.Name)):
                    self.set_mapping['expression'] = ast.get_source_segment(source, name_node)
                else:
                    asset_name = ([name_node.value] if isinstance(name_node, ast.Constant)
