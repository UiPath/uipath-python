...
def generate_content_types_content():
    templates_path = os.path.join(os.path.dirname(__file__), 'templates', '[Content_Types].xml.template')
...
def generate_nuspec_content(projectName, packageVersion, description):
    ...
    templates_path = os.path.join(os.path.dirname(__file__), 'templates', 'package.nuspec.template')
...
def generate_rels_content(nuspecPath, psmdcpPath): 
    ...
    templates_path = os.path.join(os.path.dirname(__file__), 'templates', '.rels.template')
...
def generate_psmdcp_content(projectName, version, description):
    templates_path = os.path.join(os.path.dirname(__file__), 'templates', '.psmdcp.template')
    creator = "UiPath"
...
def pack(projectName, description, type, version, directory):
    ...
    [psmdcp_file_name, psmdcp_content] = generate_psmdcp_content(projectName, version, description)
    nuspec_content = generate_nuspec_content(projectName, version, description)
    rels_content = generate_rels_content(f"/{projectName}.nuspec", f"/package/services/metadata/core-properties/{psmdcp_file_name}")
...
@click.command()
@click.option("--directory", prompt="Target Directory", default=".", help="The directory of your project")
@click.option("--version", prompt="Version", default="1.0.0", help="Version of this project")
def hello(directory, version):
    config = check_config_file(directory)
    click.echo(f"Packaging project {config['project_name']}:{version} description {config['description']} and type {config['type']}")
    pack(config['project_name'], config['description'], config['type'], version, directory)
